...
export const t7mMiddleware = <TEnv extends Env>() =>
    createMiddleware<TEnv>(async (c, next) => {
        ...
        c.transform = function <TInput, TOutput, U extends ContentfulStatusCode = ContentfulStatusCode>(
            object: TInput,
            ...
            const transformed = transformer.transform({ input: object, includes: includes as any })
        ...
        c.transformMany = function <TInput, TOutput, U extends ContentfulStatusCode = ContentfulStatusCode>(
            objects: TInput[],
            transformer: AbstractTransformer<TInput, TOutput, any, any>,
            status?: U,
            headers?: HeaderRecord
            ...
            const includes = include?.split(',')
            const transformed = transformer.transformMany({ inputs: objects, includes: includes as any })
            return c.json(transformed, status, headers)
        }
